...
class Order(Gclass):
    ...
    def __init__(self, id, cost, order_date, company_id, warehouse_id):
        ...
        self._cost = int(cost) if cost is not None and str(cost).strip() != 'None' else 0
        self._order_date = order_date
        self._company_id = int(company_id) if company_id is not None and str(company_id).strip() != 'None' else 0
        self._warehouse_id = int(warehouse_id) if warehouse_id is not None and str(warehouse_id).strip() != 'None' else 0
        Order.obj[id] = self
        Order.lst.append(id)
